...
## 실전 프로젝트: YouTube 영상 객체 인식

### Google Colab에서 YouTube 영상 분석하기

YouTube 영상을 다운로드하여 YOLOv11로 객체를 인식하는 실습 코드입니다!
...
# YouTube URL 설정 (여기만 변경하세요!)
YOUTUBE_URL = "https://www.youtube.com/watch?v=dQw4w9WgXcQ"

# 영상 다운로드 (30초만)
!yt-dlp -f "best[ext=mp4]" "{YOUTUBE_URL}" -o "video.mp4" --quiet
!ffmpeg -i video.mp4 -t 30 -c copy short.mp4 -y -loglevel quiet

# YOLOv11 객체 인식
model = YOLO('yolo11n.pt')
results = model.predict('short.mp4', save=True, conf=0.5)

# 결과 보기
from IPython.display import Video, display
display(Video('runs/detect/predict/short.mp4', width=640))
```
...
1. **yolov11_youtube_simple.py** - 간단한 실행 코드 (초보자용 ⭐)
2. **yolov11_youtube_detection.py** - 상세한 분석 코드 (고급 기능)
3. **README_YouTube_Detection.md** - 자세한 사용 설명서

각 파일의 자세한 사용법은 `README_YouTube_Detection.md`를 참고하세요!
